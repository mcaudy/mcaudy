...
rule trigger_phylogenetic_rebuild:
    message: "Triggering ncov GitHub Actions via repository dispatach {params.dispatch_type}"
    input:
        "logs/upload.txt" # just a proxy for a successful `upload` rule
    log:
    ...
    run:
        ...
        if config["S3_DST_BUCKET"] == "nextstrain-data/files/ncov/open":
            ...
            raise Exception("Rule trigger_phylogenetic_rebuild cannot be run with a non-defaule S3_DST_BUCKET")
        import requests
        import json
        headers = {
        ...
        data = json.dumps(
            {"event_type": params.dispatch_type}
        )
